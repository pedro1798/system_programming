...
int main(int ac, char* argv[]) {
    ...
    /* 타임아웃 타이머 생성 */
    make_timer(&timer_timeout, SIGUSR1, 3, 0);
...
void make_timer(timer_t *timerID, int signo, int sec, int interval_sec) {
    ...
    // 1. 시그널 이벤트 설정 (어떤 시그널을 보낼지)
    sev.sigev_notify = SIGEV_SIGNAL;
    sev.sigev_signo = signo;
    sev.sigev_value.sival_ptr = timerID; // 핸들러에서 타이머 ID 확인 가능
    ...
    // 3. 시간 설정
    // it_value: 타이머가 처음 만료될 시간 (초기 지연)
    its.it_value.tv_sec = sec;
    its.it_value.tv_nsec = 0;

    // it_interval: 타이머가 만료된 후 반복될 주기 (0이면 반복 안 함)
